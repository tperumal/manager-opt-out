...
//
// Setup:
// 1. Open your Google Sheet (e.g., "Manager Opt-Outs")
...
// 6. Copy the Web App URL and paste it into index.html as SCRIPT_URL
//
...
function doGet(e) {
  return ContentService
    .createTextOutput(JSON.stringify({ status: 'ok', message: 'Opt-out endpoint is running. Use POST to submit.' }))
  ...
  try {
    ...
    for (var i = 1; i < existing.length; i++) { // skip header row
      if (String(existing[i][1]).replace(/\D/g, '') === phone) {
        return buildResponse({ success: true, message: 'Already opted out' });
      }
    }
    ...
    return buildResponse({ success: true, message: 'Opt-out recorded' });
  } catch (err) {
